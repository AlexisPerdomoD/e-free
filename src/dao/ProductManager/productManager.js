...
export default class ProductManager {
    ...
    async getProductsPaginate(querys) {
        ...
        let { limit, page, sort, category } = querys
        ...
        if (limit && limit < payload) {
            ...
            if (page && page - 1 > 1) {
                response.products = response.products.slice(
                    (page - 1) * limit - 1,
                    page * limit > products.length - 1 ? products.length - 1 : page * limit
                )
            } else {
    ...
    async updateProduct(id, updates) {
        try {
            ...
            if (updates.code && m.products.find((product) => product.code === updates.code))
                throw em.createError({
                    name: 'CastError',
                    status: 400,
                    message: 'this code is already in other product, need a newone',
                    code: ErrorCode.GENERAL_USER_ERROR,
